from two_d_version import attention
...
def model():
    #day1
    input_1 = [Input(shape=(3, 3, 5), name='input_day_1_{0}'.format(i)) for i in range(0, 2)]

    conv_1 = [Conv2D(filters=5, kernel_size=(3, 3), input_shape=(3, 3, 5), name='Conv_day_1_{0}'.format(i))(input_1[i])
              for i
              in range(0, 2)]

    conv_1 = [Reshape((5, 1))(conv_1[i]) for i in range(0, 2)]

    conv1 = Concatenate(axis=-1)([conv_1[i] for i in range(0, 2)])

    lstm1 = LSTM(2, input_shape=(5, 2), return_sequences=True, name='att_lstm_day_1')(conv1)


    # day2
    input_2 = [Input(shape=(3, 3, 5), name='input_day_2_{0}'.format(i)) for i in range(0, 2)]

    conv_2 = [Conv2D(filters=5, kernel_size=(3, 3), input_shape=(3, 3, 5), name='Conv_day_2_{0}'.format(i))(input_2[i])
              for i
              in range(0, 2)]

    conv_2 = [Reshape((5, 1))(conv_2[i]) for i in range(0, 2)]

    conv2 = Concatenate(axis=-1)([conv_2[i] for i in range(0, 2)])

    lstm2 = LSTM(2, input_shape=(5, 2), return_sequences=True, name='att_lstm_day_2')(conv2)


    # day3
    input_3 = [Input(shape=(3, 3, 5), name='input_day_3_{0}'.format(i)) for i in range(0, 2)]

    conv_3 = [Conv2D(filters=5, kernel_size=(3, 3), input_shape=(3, 3, 5), name='Conv_day_3_{0}'.format(i))(input_3[i])
              for i
              in range(0, 2)]

    conv_3 = [Reshape((5, 1))(conv_3[i]) for i in range(0, 2)]

    conv3 = Concatenate(axis=-1)([conv_3[i] for i in range(0, 2)])

    lstm3 = LSTM(2, input_shape=(5, 2), return_sequences=False, name='att_lstm_day_3')(conv3)
    ...
    att_lstm1 = Reshape(target_shape=(1, 2))(att_lstm1)
    att_lstm2 = attention.Attention(method='cba', name='Attention2')([lstm2, lstm3])
    att_lstm2 = Reshape(target_shape=(1, 2))(att_lstm2)
    ...
    ###################################
    att_high_level = LSTM(2, input_shape=(2, 2), return_sequences=False, name='att_lstm')(att_lstm)
    ###################################
    ...
    model = Model(inputs=[input_1[0],input_1[1],input_2[0],input_2[1],input_3[0],input_3[1],knn_tensor_input], outputs=pred)
